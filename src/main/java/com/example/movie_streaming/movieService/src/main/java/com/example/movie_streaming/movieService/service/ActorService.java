...
public class ActorService {
    ...
    public Page<ActorFullResponse> getAllActors(Pageable pageable, String keyword) {
        ...
        return actors.map(actor -> {
            Set<MovieActor> movieActors = new HashSet<>(actor.getMovieActors());
            List<MovieResponse> movieResponses = movieMapper.mapMovieActorsToMovieResponses(movieActors);
            return actorMapper.toFullResponse(actor, movieResponses);
        });
    }

    @Transactional(readOnly = true)
    public ActorFullResponse getById(Long id) {
