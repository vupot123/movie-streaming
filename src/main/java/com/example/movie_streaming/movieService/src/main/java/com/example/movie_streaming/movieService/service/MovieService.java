...
import com.example.movie_streaming.movieService.model.dto.request.CreateSeasonRequest;
...
public class MovieService {
    ...
    public Page<MovieResponse> getAllOrSearch(String keyword, int page, int size) {
        ...
        if (keyword != null && !keyword.trim().isEmpty()) {
            movies = movieRepository.searchByTitleOrActorName(keyword.trim());
        } else {
        ...
        List<MovieResponse> movieResponses = movies.subList(start, end).stream()
                .map(movieMapper::toResponse)
                .toList();
    ...
    public MovieResponse createMovie(CreateMovieRequest request) {
        ...
        if (movieRepository.existsByTitleIgnoreCase(normalizedTitle)) {
            throw new IllegalArgumentException("Phim với tiêu đề '" + request.getTitle() + "' đã tồn tại");
        }
        ...
        if (request.getGenreNames() != null) {
            for (String name : request.getGenreNames()) {
                genreRepository.findByNameIgnoreCase(name.trim())
                        .ifPresentOrElse(
                                genre -> genreIds.add(genre.getId()),
                                () -> {
                                    throw new IllegalArgumentException("Thể loại không tồn tại: " + name);
                                }
                        );
            }
        }
        ...
        if (request.getCollections() != null && !request.getCollections().isEmpty()) {
            List<CollectionMovie> collectionMovies = request.getCollections().stream()
                    ...
                    .map(cid -> {
                        ...
                        return new CollectionMovie(
                                new CollectionMovieId(movie.getId(), cid),
                                collection,
                                movie
                        );
                    })
                ...
                if (s.getEpisodes() != null) {
                    List<Episode> episodes = s.getEpisodes().stream()
                            .map(e -> Episode.builder()
                                    ...
                                    .build())
                            .toList();
                    Set<Episode> savedEpisodes = new HashSet<>(episodeRepository.saveAll(episodes));
                    savedSeason.setEpisodes(savedEpisodes); // ✅ OK

                }

                allSeasons.add(savedSeason);
            }
            movie.setSeasons(new HashSet<>(allSeasons)); // ✅ OK

        }

        saveMovieRelations(savedMovie, new ArrayList<>(allActorIds), genreIds, countryId,
    ...
    private boolean updateMovieFields(Movie movie, UpdateMovieRequest request) {
        ...
        if (request.getCountryName() != null && !request.getCountryName().isBlank()) {
            Country country = countryRepository.findByNameIgnoreCase(request.getCountryName().trim())
                    .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy quốc gia: " + request.getCountryName()));
    ...
    private void saveMovieRelations(Movie movie, List<Long> actorIds, List<Integer> genreIds, Integer countryId, String smallBanner, String largeBanner) {
        ...
        if ((smallBanner != null && !smallBanner.trim().isBlank()) || (largeBanner != null && !largeBanner.trim().isBlank())) {
    ...
    private void deleteMovieRelations(Long movieId) {
        // 1. Lấy tất cả các seasons của movie
        List<Season> seasons = seasonRepository.findByMovieId(movieId);

        // 2. Xoá toàn bộ episodes theo season
